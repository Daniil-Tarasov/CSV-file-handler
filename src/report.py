...
def generate_average_gdp_report(country_gdps):
    ...
    headers = ["country", "gdp"]
    table_data = [[country, f"{avg_gdp}"] for country, avg_gdp in avg_data]

    return tabulate(table_data, headers=headers, tablefmt="grid", floatfmt=".2f")


def generate_report(country_gdps, report_type):
    """Расширяемая функция для разных типов отчетов."""
    reports = {"average-gdp": generate_average_gdp_report}
